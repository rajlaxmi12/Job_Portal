  ...
  return (
    <div>
      ...
      <div className='flex items-center justify-center w-screen my-5'>
        <form onSubmit={submitHandler} className='p-8 max-w-4xl bg-white border border-gray-200 shadow-lg rounded-md'>
          ...
          </div>
          {
            loading ? <Button className='w-full my-4 bg-[#6A38C2]'><Loader2 className='mr-2 h-4 w-4 animate-spin' />Please wait...</Button>
              : <Button type='submit' className='w-full my-4 bg-[#6A38C2] hover:bg-[#5b30a6]'>Post New Job</Button>
          }
          {
            companies.length === 0 && <p className='text-sm text-red-500 font-bold text-center my-3'>Please register company first, before posting</p>
          }
        </form>
      </div>
    </div>
